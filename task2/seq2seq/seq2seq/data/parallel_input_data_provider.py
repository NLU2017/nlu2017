...
def make_parallel_input_data_provider(data_sources_source,
                                data_sources_target,
  ...
  dataset_source1 = tf.contrib.slim.dataset.Dataset(
      data_sources=data_sources_source[0],
      reader=reader,
      decoder=decoder_source,
      num_samples=num_samples,
      items_to_descriptions={})

  dataset_source2 = tf.contrib.slim.dataset.Dataset(
      data_sources=data_sources_source[1],
      reader=reader,
      decoder=decoder_source,
      num_samples=num_samples,
      items_to_descriptions={})
  ...
  return ParallelInputDataProvider(
      dataset1=dataset_source1, dataset2 = dataset_source2, dataset3=dataset_target, **kwargs)


class ParallelInputDataProvider(data_provider.DataProvider):
  ...
  def __init__(self,
    ...
    _, data_source1 = parallel_reader.parallel_read(
    ...
    _, data_source2 = parallel_reader.parallel_read(
    ...
    if shuffle:
      shuffle_queue = tf.RandomShuffleQueue(
          ...
          seed=seed)
      enqueue_ops = []
      enqueue_ops.append(shuffle_queue.enqueue([data_source1, data_source2, data_target]))
    ...
    if dataset3 is not None:
      ...
      tensors = tensors1 + tensors2 + tensors_t

    super(ParallelInputDataProvider, self).__init__(
